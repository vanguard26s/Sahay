/**
 * ResQ-IQ: Unified Disaster Intelligence & Response Command Center
...
window.computeAndDrawSafeRoute = async function() {
    ...
    // Pick origin (e.g. stranded area) and destination (safe base camp)
...
async function fetchSitRepAndUpdate() {
    try {
        ...
        if (res.ok) {
            ...
            // Render disaster distribution bars
...
// --- Window Global Event Handlers for Buttons/Popups ---
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("btnSyncUsgs").addEventListener("click", async () => {
        ...
        try {
            const res = await fetch("/api/sync-usgs", { method: "POST" });
            const data = await res.json();
